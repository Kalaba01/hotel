function noviRed() {
    ...
    const columns = ['zaglavlje_id', 'kupac', 'pocetni_datum', 'krajnji_datum', 'status', 'napomena'];
...
function sacuvajJSON() {
    ...
    xhr.open('POST', 'http://localhost:5000/alotman/sacuvaj-zaglavlje', true);
    xhr.setRequestHeader('Content-Type', 'application/json');

    xhr.onload = function () {
        if (xhr.status === 200) {
            console.log('Podaci su uspešno sačuvani u bazi.');
            window.location = "http://localhost:5000/alotman/stavke";
        } else {
